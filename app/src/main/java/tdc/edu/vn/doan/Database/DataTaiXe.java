...
public class DataTaiXe {
    ...
    public TaiXe getFisrt(String ma){
        SQLiteDatabase db=handler.getReadableDatabase();
        TaiXe taiXe =null;
        String sql="SELECT * from "+ Table_TaiXe.TABLE_NAME + "WHERE " +Table_TaiXe.KEY_MATAIXE +"="+ ma;
        Cursor cursor=db.rawQuery(sql,null);
        if(cursor.moveToFirst()){
               int id=cursor.getInt(0);
                String maTaiXe=cursor.getString(1);
                ...
                String diaChi=cursor.getString(4);
                byte[] chuKy=cursor.getBlob(5);
            taiXe =new TaiXe(id,maTaiXe,ten,ngaySinh,diaChi,chuKy);
            cursor.close();
        }
        return taiXe;
    }

    public TaiXe getTXbyId(int idTX){
        SQLiteDatabase db=handler.getReadableDatabase();
        TaiXe taiXe;
        String sql="SELECT * from "+ Table_TaiXe.TABLE_NAME + " WHERE " +Table_TaiXe.KEY_ID +"="+ idTX;
        Cursor cursor=db.rawQuery(sql,null);
        if(cursor.moveToFirst()){
            int id=cursor.getInt(0);
            String maTaiXe=cursor.getString(1);
